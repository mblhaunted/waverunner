...
class Board:
    ...
    # Ralph loop: ordered list of validation steps run after each implementation task.
    # Each step is an independent shell command. ALL must pass (exit 0) for the task to be done.
    # Generated by planning team to match what they're building.
    # Examples: ["cargo build", "npm run typecheck", "pytest tests/unit/"]
    validate_steps: list[str] = field(default_factory=list)

    # Legacy single-command field — kept for backward compat with saved boards.
    # Migrated to validate_steps on load if validate_steps is empty.
    validate_cmd: str = ""
    ...
    def from_dict(cls, data: dict) -> "Board":
        ...
        board.validate_cmd = data.get("validate_cmd", "")
        # Load validate_steps; migrate legacy validate_cmd if steps list is absent
        loaded_steps = data.get("validate_steps", [])
        if loaded_steps:
            board.validate_steps = loaded_steps
        elif board.validate_cmd:
            board.validate_steps = [board.validate_cmd]
        return board
